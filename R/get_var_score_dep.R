...
#' belonging to the same transition type and computes the variability score. The score is the variance
#' of all the gaps, divided by their mean. 
#'
...
get_var_score_dep <- function(gap_table){
  ...
  norm_gaps = gap_table$gap_dur/  mean(gap_table$gap_dur)
  
  #account for case when there is only one occurance of a transition
  if(length(norm_gaps)==1){
